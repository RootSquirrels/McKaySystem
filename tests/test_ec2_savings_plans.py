...
def test_savings_plans_coverage_gap_emits() -> None:
    ...
    assert (f.dimensions or {}).get("optimization_focus") == "ec2_instance_sp_candidate"
...
def test_savings_plans_utilization_low_emits() -> None:
    ...
    assert (f.dimensions or {}).get("optimization_focus") == "future_commitment_reduction"
...
def test_savings_plans_coverage_gap_prefers_compute_for_mixed_fleet() -> None:
    import checks.aws.ec2_savings_plans as mod
            ...
            "describe_instances": [
                {
                    "Reservations": [
                        {
                            "Instances": [
                                _running_instance("i-1", "m5.large"),
                                _running_instance("i-2", "c6i.large"),
                            ]
                        }
                    ]
                }
    ...
    sp = FakeSavingsPlansClient(pages=[{"savingsPlans": []}])
    pricing = FakePricingByField({"m5.large": 0.10, "c6i.large": 0.12}, field_name="instanceType")

    checker = EC2SavingsPlansChecker(
        account=mod.AwsAccountContext(account_id="123", billing_account_id="123"),
    )
    findings = list(checker.run(make_run_ctx(ec2=ec2, savingsplans=sp, pricing=pricing)))
    hit = next(f for f in findings if f.check_id == "aws.ec2.savings.plans.coverage.gap")
    assert hit.dimensions["optimization_focus"] == "compute_sp_candidate"


def test_savings_plans_utilization_low_prefers_compute_rebalance_for_ec2instance_only() -> None:
    import checks.aws.ec2_savings_plans as mod
    ...
    findings = list(checker.run(make_run_ctx(ec2=ec2, savingsplans=sp, pricing=pricing)))
    hit = next(f for f in findings if f.check_id == "aws.ec2.savings.plans.utilization.low")
    assert hit.dimensions["optimization_focus"] == "future_compute_sp_rebalance"
